...
######################## MODEL FROM Soft Layer Ordering PAPER ########################


def learned_sharing(task_outputs, num_modules):
    nt = len(task_outputs)
    return nn.Sequential(
        LearnedTaskRouting(nt, [conv_layer(1, 53) for _ in range(num_modules)]),
        LearnedTaskRouting(nt, [conv_layer(53, 53) for _ in range(num_modules)]),
        LearnedTaskRouting(nt, [conv_layer(53, 53) for _ in range(num_modules)]),
        LearnedTaskRouting(nt, [conv_layer(53, 53) for _ in range(num_modules)]),

        StaticTaskRouting(nt, [decoder(53, s) for s in task_outputs]),
    )
...
def no_sharing(task_outputs, num_modules):
    nt = len(task_outputs)
    return nn.Sequential(
        StaticTaskRouting(nt, [conv_layer(1, 53) for _ in task_outputs]),
        StaticTaskRouting(nt, [conv_layer(53, 53) for _ in task_outputs]),
        StaticTaskRouting(nt, [conv_layer(53, 53) for _ in task_outputs]),
        StaticTaskRouting(nt, [conv_layer(53, 53) for _ in task_outputs]),
...
def prior_sharing(task_outputs, num_modules):
    nt = len(task_outputs)
    return nn.Sequential(
        IgnoreTaskRouting(conv_layer(1, 53)),
        IgnoreTaskRouting(conv_layer(53, 53)),
        StaticTaskRouting(nt, [conv_layer(53, 53) for _ in task_outputs]),
        StaticTaskRouting(nt, [conv_layer(53, 53) for _ in task_outputs]),
